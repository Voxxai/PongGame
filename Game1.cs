...
public class Game1 : Game
{
    ...
    protected override void Initialize()
    {
        ...
        _scores = new int[4];
        _lives = new[] { 5, 5, 5, 5 }; // Initialize lives to 5 for all paddles
    ...
    protected override void LoadContent()
    {
        ...
        var ballColor = new Color[10 * 10];
        for (var i = 0; i < ballColor.Length; i++) ballColor[i] = Color.White;
        ...
        _boundaryTexture.SetData(new[] { Color.White });
    ...
    private void UpdateBall()
    {
        ...
        var ballRect = new Rectangle((int)_ballPosition.X, (int)_ballPosition.Y, 10, 10);

        // Ball collision with paddles
        var hitPaddle = false;
        for (var i = 0; i < _paddles.Length; i++)
            if (_paddles[i] != Rectangle.Empty && _paddles[i].Intersects(ballRect))
            {
                ...
                var dotProduct = Vector2.Dot(_ballVelocity, normal);
        ...
        if (_ballPosition.X < _boundary.Left &&
            IsSideSolid(2)) _ballVelocity.X *= -1; // Reflect off the solid left wall

        if (_ballPosition.X + 10 > _boundary.Right &&
            IsSideSolid(3)) _ballVelocity.X *= -1; // Reflect off the solid right wall

        if (_ballPosition.Y < _boundary.Top && IsSideSolid(0)) _ballVelocity.Y *= -1; // Reflect off the solid top wall

        if (_ballPosition.Y + 10 > _boundary.Bottom &&
            IsSideSolid(1)) _ballVelocity.Y *= -1; // Reflect off the solid bottom wall
    ...
    {
        var aiSpeed = 7f; // Adjust this value to control the AI speed
        ...
        for (var i = 0; i < _paddles.Length; i++)
            if (!_isPlayerControlled[i])
            {
                if (i < 2) // Top or Bottom paddle
                {
                    var targetX = _ballPosition.X - _paddles[i].Width / 2;
                    _paddles[i].X += (int)MathHelper.Clamp(targetX - _paddles[i].X, -aiSpeed, aiSpeed);
                }
                else // Left or Right paddle
                {
                    var targetY = _ballPosition.Y - _paddles[i].Height / 2;
